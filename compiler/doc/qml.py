...
class Component(object):
	...
	def generate_section(self, r, type, title, data):
		...
		for value in values:
			r.append('\t\tDocElement {')
			r.append('\t\t\tname: "%s"; ' %value.name)
			if value.doc is not None:
				r.append('\t\t\ttext: "%s";' %value.doc.text)
			r.append('\t\t}')
			r.append('')
...
class Documentation(object):
	def __init__(self, root):
		self.root = root
		self.qmlroot = os.path.join(root, 'qml')
		self.packages = {}
	...
	def generate_component(self, package, name, component):
		...
		with open(os.path.join(self.qmlroot, package, name + '.qml'), 'wt') as f:
			f.write(component.generate(self))

	def generate(self):
		if not os.path.exists(self.qmlroot):
			os.makedirs(self.qmlroot)
		...
		for package, components in sorted(self.packages.iteritems()):
			...
			path = os.path.join(self.qmlroot, package)
			if not os.path.exists(path):
				os.mkdir(path)

			for name, component in components.iteritems():
				package_toc.append('\tComponent { name: "%s.%s"; path: "%s/%s.qml"; }' %(package, name, package, name))
				toc.append("- '%s.%s': '%s/%s.md'" %(package, name, package, name))
				self.generate_component(package, name, component)

			with open(os.path.join(self.qmlroot, package + '.qml'), 'wt') as f:
				f.write('\n'.join(package_toc))
                                f.write('\n}\n')

		with open(os.path.join(self.qmlroot, 'mkdocs.qml'), 'wt') as f:
			f.write('\n'.join(toc))
