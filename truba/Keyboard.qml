	...
	ListView {
		...
		delegate: ListView {
			...
			delegate: Rectangle {
				...
				color: activeFocus ? colorTheme.activeFocusColor : (model.contextColor ? model.contextColor : colorTheme.focusablePanelColor);
				...
				Image {
					anchors.centerIn: parent;
					source: colorTheme.res + "keyboard/" + (parent.activeFocus ? "b_" : "") + model.icon;
					visible: model.icon;
				}
			}
